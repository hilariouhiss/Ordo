...
- New Tauri plugins or capabilities must be added to `src-tauri/capabilities/default.json` (currently `core:default`, `core:window:allow-hide`, `notification:default`, `opener:default`); otherwise the frontend cannot call them. Plugins driven entirely from Rust (tray, global shortcut) need no entry.
