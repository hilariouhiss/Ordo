...
describe("the brand mark", () => {
   ...
   *
   *   logo.svg, logo-square.svg   the supplied export — the mark with its
   *                               background removed — byte-identical copies.
   *                               The sidebar renders the first; this is also
   *                               the favicon, the source of the packaged icon
   *                               set and the source of both runtime icons.
   *   logo-dark.svg               the same pixels with the ink swapped for the
   *                               dark theme, generated by
   *                               `scripts/gen-logo-assets.mjs`. Geometry must
   *                               be IDENTICAL — only ink may differ, or it is a
   *                               different drawing.
   *
   * The export embeds a 500x500 RGBA bitmap instead of SVG paths, so what is
   * checked here is that bitmap and the two icons rasterised from it. The
   * window, taskbar and tray icon is those two files, one per theme
   * (src-tauri/src/icons.rs); `cargo test --lib icons` checks the same pair from
   * the other side.
   */
  ...
  /** The embedded bitmap's header: width, height, depth and colour type. */
  const embedded = (svg: string) => {
    const base64 = svg.match(/base64,([A-Za-z0-9+/=]+)/)?.[1] ?? "";
    // `atob` gives one character per byte, which is all a PNG header needs.
    const png = atob(base64);
    const u32 = (at: number) =>
      ((png.charCodeAt(at) << 24) |
        (png.charCodeAt(at + 1) << 16) |
        (png.charCodeAt(at + 2) << 8) |
        png.charCodeAt(at + 3)) >>>
      0;
    return { width: u32(16), height: u32(20), depth: png.charCodeAt(24), colour: png.charCodeAt(25) };
  };
  ...
  it("keeps the light and square copies byte-identical", () => {
    ...
    // Pin the artwork. If this fails, someone replaced the supplied export: that
    // may be deliberate, but it needs a regenerated dark copy and icon set
    // (`node scripts/gen-logo-assets.mjs`) rather than passing silently.
    expect(embedded(read("logo.svg")), "the supplied export changed shape").toEqual({
      width: 500,
      height: 500,
      depth: 8,
      colour: 6,
  ...
  it("re-inks the dark copy without moving a single pixel", () => {
    expect(embedded(read("logo-dark.svg")), "the dark copy is a different export").toEqual(
      embedded(read("logo.svg")),
    );
    // Same wrapper, so the transform and the canvas are the export's own.
    const wrapper = (svg: string) => svg.slice(0, svg.indexOf("base64,"));
    expect(wrapper(read("logo-dark.svg"))).toBe(wrapper(read("logo.svg")));
  });
