...
| 9 | R7c | 拖拽：任务 → 任务（变成其子任务） | 大 | 取决于方案（见 §5.1） | 任务列表 + 数据模型决策 |
...
- R7 拆成 3 个独立可交付的步骤；R7c 依赖 §5.1 的产品决策，可能顺延。
...
**R7c 任务 → 任务 = 子任务（大，且有损）**
见 §5.1 的两个选项；不论选哪个，都需要：拖到任务行上时高亮该行、落下后刷新两个任务、以及一条可撤销/可说明的反馈。

**长按语义**：原生 Drag API 没有长按。做法是 `pointerdown` 起 250–300ms 计时，到点才把行标记为 `draggable`（Chromium 在拖拽起点才读取该属性，所以在此之前松手仍是普通点击/展开）。鼠标是否也走长按、还是鼠标立即拖而仅触摸长按，是一个常量的事（§5.2）。
...
| P3 | R2 + R6 | 项目维度无截止日期；5 个日期控件空值文案统一为「年/月/日 时:分」 |
| P4 | R7a | 项目可拖入/拖出命名空间 |
| P5 | R7b | 任务可拖到其他项目 |
| P6 | R7c | 任务可拖成另一个任务的子任务（待 §5 决策） |
...
1. **R7c 的语义**：(A) 复制 + 软删 —— 在目标任务下建同标题/备注/优先级/截止/复杂度的子任务，再软删源任务；标签、评论、计时、依赖、重复规则**不带过去**（软删可恢复）。(B) 给 `tasks` 加 `parent_task_id` 真层级 —— 迁移 + 模型 + 服务 + 列表/看板/筛选/统计全链路，且「子任务」与「任务」出现两种表达同一关系的模型。推荐 A。
2. **长按**：鼠标也长按 300ms，还是鼠标立即拖、仅触摸/触控板长按？
