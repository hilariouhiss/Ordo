...
| 2026-09-15 | 评审 R7：长按取「鼠标立即拖、触摸长按」；R7c 取真层级字段 `tasks.parent_task_id`，需先设计（§7） |
| 2026-09-15 | P3（R2+R6）落地：项目 `due_at` 全栈移除 + `V6` 迁移；`DateField` 统一 5 个日期控件（工具类 `child-indent`、`DateField` 均为共享件） |

## 7. R7c 设计前提（待定案后再实现）

用户选择用**真层级字段**而不是「复制 + 软删」，于是这次改动落在数据模型上，动手前需要先把下面几条定下来：

1. **与 `subtasks` 的关系**：`subtasks` 是一张独立表（标题/备注/优先级/截止/复杂度/done/sort_order），任务层级则是 `tasks.parent_task_id` 自引用。两者并存意味着「一个任务的下级」有两种表达。可选：
   - (a) **并存**：`parent_task_id` 只表示「任务挂在任务下」，`subtasks` 继续做任务内部的检查项；拖到任务上时把源任务改成子任务（保留标签/评论/计时/依赖，这正是选真层级的目的）。落地面：`tasks` 迁移 + 模型/IPC 类型、`task:update` 允许改 parent、列表/看板/筛选/统计的可见性与排序、删除父任务时的级联策略。
   - (b) **合并**：让 `subtasks` 迁移成 `tasks` 的一层（把子任务行升级为任务行，`parent_task_id` 指向父任务），此后只有一种层级。落地面更大，且要迁移既有子任务数据、重写子任务相关命令与 UI。
2. **级联与可见性**：软删/归档父任务时子任务是否跟着消失；「今天/即将到来/收件箱」这些视图是否把有父任务的任务排除（否则同一件事会出现在两处）；项目进度与看板是否统计子任务。
3. **深度**：允许无限嵌套还是只允许一层（与 `subtasks` 并存时，无限嵌套会让缩进与排序键的复杂度上升）。
4. **拖放规则的冲突**：任务行目前既是拖源（R7b 移到项目），落下目标又可能是任务行（R7c 变成子任务）——同一次拖拽里「落到项目行」与「落到任务行」要有不同的落点高亮与语义，且都落在同一个列表里。

**在 §7.1–§7.3 定案前不动 R7c 的代码。**
