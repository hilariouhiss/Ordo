...
**状态**：全部落地。R1–R9 已按 §4 的 P1–P6 逐条实现（R8/R9 为 2026-09-15 之后的追加），每条一个提交；P6（R7c）的落地说明与提交点 hash 见 §7。
**范围**：R1–R9，均为 M0–M9 已交付功能之上的增量变更，不新增子系统
...
| 10 | R8 | 展开箭头不再把父行推到子项那一列 | 小 | 无 | `src/app/AppShell.tsx` |
| 11 | R9 | 侧边栏项目行可展开，列出未完成任务 | 小–中 | 无 | `AppShell.tsx`、`app-shell-sidebar.test.tsx` |
...
**R7c 任务 → 任务 = 子任务（大）**
已落地：`tasks` 加自引用层级字段（不是「复制 + 软删」），拖过去保留标签/评论/计时/依赖。落点规则与语义细节见 §7；实现上拖到任务行会高亮该行，落点不可落时不高亮、松手无变化。
...
**现状**：侧边栏命名空间行 = `[chevron] [Link]`，chevron 用共享的 `iconButtonClass`（`size-7` = 28px）+ `gap-0.5`（2px）= **30px**；子项 nav 用 R1 的 `child-indent`（`margin-left 20 + border 1 + padding-left 9`）= **30px**。两者像素级相等，所以父行自己的图标和它子项的图标落在同一列（都在 48px），层级读不出来。

**目标**：「列表展开、子元素缩进、父元素不缩进」——展开箭头不能改变它所属那一行的内容列。

**落地**：箭头收到 `w-5`（20px）并放进一个**独立的 20px 槽位**，行再用负外边距把自己向左拉回该槽位的宽度，于是有无箭头都不动内容列（`AppShell.tsx` 的 `treeRowClass` / `TREE_CONTENT_CLASS` / `Disclosure` / `DisclosureSpacer`）：

| 层级 | 内容列 | 槽位 |
|------|--------|------|
| 0（命名空间行、根级项目行） | 48px → **22px** | `-ml-2`，落在侧边栏自己的 8px gutter 里（与上方导航图标 18px 同列） |
| 1（命名空间下的项目行） | **48px 不变** | `-ml-2.5`，正好落在子列表的引导线上 |
| 2（项目下的任务行） | ≈74px | 无箭头 |

槽位恒在（`DisclosureSpacer`），所以有子项和无子项的行仍然对齐。`child-indent` 令牌与 `SubtaskRow` 的行式缩进不受影响（R1 的 20px 步长未变）。

**复杂度**：小（纯样式 + 一处组件化，`AppShell.tsx` 一个文件）。

---

### R9 侧边栏项目行可展开，列出未完成任务

**现状**：侧边栏只有命名空间行有展开箭头；项目行是纯导航，看不到任务。壳层 `onMount` 也只加载命名空间与项目（任务要等某个任务视图挂载后才进 store）。

**目标**：项目行可展开，列出该项目的未完成任务。

**落地**：

- 项目行复用 R8 的箭头槽位，`ProjectItem` 承担「行 + 展开 + 任务列表」；展开状态是组件内的本地 signal（与 `collapsedGroups`、`archivedOpen` 同级：视图状态，不落库），**默认收起**。
- 列出口径：`projectId === 该项目 && parentTaskId === null && !completedAt`（顶层未完成）。子任务不单列——它在别处已经以「父任务的孩子」或「带前缀的顶层行」出现，侧边栏也没有树的宽度。项目没有未完成任务时不渲染箭头（留 `DisclosureSpacer`），避免一个点不开的死箭头。
- 任务行点击打开既有的任务详情弹窗（`openTaskViewer`）。侧边栏收起成 rail 时不渲染箭头与任务列表（与命名空间行一致）。
- 已归档区的项目行复用同一组件，因此同样可展开。
- 壳层 `onMount` 增加 `if (!tasksState.loaded) void loadTasks()`（与 `useViewData` 同一个一次性守卫），否则首屏侧边栏拿不到任务。
- 未做：项目行不显示未完成数、任务行不是拖源、没有「全部展开」。

**复杂度**：小–中（`AppShell.tsx` + `app-shell-sidebar.test.tsx`）。
...
| P4 | R7a | 项目可拖入/拖出命名空间 ✅ |
| P5 | R7b | 任务可拖到其他项目 ✅ |
| P6 | R7c | 任务可拖成另一个任务的子任务（真层级字段，见 §7）✅ |
...
无。§7 的四个核心问题（与 `subtasks` 的关系、深度、视图可见性、落点规则）已定案并全部落地，见 §7 的状态一行。
...
| 2026-09-15 | R7c spec 定案并出实施计划；四条视图/语义决策确认为推荐值（规则 A 行内分组、完成父任务不级联、子任务独立提醒、拖到项目行 = 移出父任务并移进项目） |
...
| 2026-09-15 | P6（R7c）落地：`subtasks` 并入 `tasks` 单层自引用，七个 `subtask:*` 退役、新增 `task:reorder`；规则 A 行内分组、完成父任务不级联、子任务独立提醒、项目行落点 = 移出父任务并移进项目。提交点 A `71937f6`、B `d10f0ae`、C 见下文 §7 |
| 2026-09-15 | R8+R9 落地：箭头改为 20px 槽位 + 负外边距（父行回到第 0 列，子项保持 `child-indent`）；项目行可展开列出顶层未完成任务，壳层补一次任务加载 |
...
**状态**：已落地（spec 与实施计划都已执行完）。三个提交点：A `71937f6`（Rust 侧全部切换 + `ARCHITECTURE.md`）、B `d10f0ae`（前端切换与 R7c 落点 + `PRD.md`，其后 `e1e4b0d` 补齐子任务行的拖源与候选）、C 即本次回写提交（`docs: record the task hierarchy milestone`）。C 的 hash 只能靠提交信息标识——一个提交写不下自己的 hash，取它用 `git log -1 --format=%h`。上面三条决策按原样实现；落地后的模型口径见 `docs/ARCHITECTURE.md` 的 V7/V8/V9 三条与 `docs/PRD.md` §2.1「子任务」，实施过程见 `docs/superpowers/specs/2026-09-15-task-hierarchy-design.md` 与 `docs/superpowers/plans/2026-09-15-task-hierarchy.md`（两份都是历史存档，不再改动）。
