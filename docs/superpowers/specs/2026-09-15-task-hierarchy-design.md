...
> §8.1 的规则 A 与 §8.6 的筛选规则已确认采用。

### 8.1 顶层列表包含哪些行

**规则 A（行内分组，已确认）**：视图先按现有谓词筛出全部匹配任务（含子任务，子任务按自己的 `dueAt`/`completedAt` 入选），然后：
...
（备选规则 B「全部顶层 + 可展开」已否决：同一视图会出现重复行，且用户无法判断哪一行是"入口"。）
...
**规则（已确认）**：按子任务**出现在哪条路径**区分，与 §8.1 的规则 A 同源：
...
**已确认**：完成父任务**不**自动完成其子任务（与"子任务独立进视图"一致：它有自己的完成状态与自己的位置）。父任务行在还有未完成子任务时照常显示进度徽标，用户可以自行决定。

代价：`已完成` 视图里可能出现「父任务已完成、子任务仍在今天」的组合。这是模型选择的必然结果，不是 bug。

### 9.2 软删 / 恢复

**已确认**：软删父任务时同一事务软删其全部子任务；恢复父任务时一并恢复。

理由：子任务的存在依附于父任务（它没有独立入口，侧边栏/视图里也只以"父任务的孩子"或"带前缀的顶层行"出现）。留下未软删的子任务会产生一批没有父行的孤儿行，而"恢复父任务"必须能把它们带回来。

### 9.3 提醒

**已确认**：子任务按自己的 `due_at` 独立提醒，**不再**因父任务完成/软删而静默（软删已由 §9.2 覆盖：子任务跟着软删，扫描自然跳过）。

这是对 `ARCHITECTURE.md:267` 现有规则的修改，需要在同一提交里改文档。

### 9.4 把子任务拖到项目行

**已确认**：允许，语义是"移出父任务 + 移进该项目"（同时清 `parentTaskId` 与 `projectId`）。落地时在通知里说明一句，避免用户以为只是换了项目。

（备选「只改项目、保留父子关系」已否决：父子会分居两个项目，与 §6.3「子任务跟随父任务的项目」冲突。）

### 9.5 重复任务

**已确认**：子任务可以有自己的重复规则，生成的下一实例继承 `parentTaskId`（照抄 `spawn_next_instance`，多传一个字段）。
