...
MVP（PRD 6.1）= M0–M3；v1 完整版（PRD 6.2）= M0–M7。
...
| D-02 | 全局快捷键快速添加 | 添加 `tauri-plugin-global-shortcut` + capabilities；全局快捷键唤起「快速添加任务」弹窗，录入即隐 | 应用无焦点时快捷键可用；录入后不影响当前工作 | T-04 |
...
8. **PRD 状态**：PRD/ARCHITECTURE 为「草案（待评审）」，评审结论若改变范围，先更新本计划再开发。
...
- 数据模型（V2 Schema）在 M0 落地后，将 ARCHITECTURE §4.2 的「实现阶段细化」占位替换为实际 DDL 摘要。
