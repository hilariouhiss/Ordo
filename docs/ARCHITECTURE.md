...
Namespace 1 ──── * Project
Project 1 ──── * Task
Task    1 ──── * Task       （子任务：单层自引用，`parent_task_id`）
...
**看板即视图（P-05）**：看板不持有任务，它把项目的任务按状态分到三列（`src/features/board/lanes.ts` 的纯函数规则，有表驱动单测）：`completedAt` 非空的任务进完成列（所以列表里勾选完成、详情弹窗里完成、拖进完成列三条路径的结果一致）；未完成的任务进 `column_id` 指向的开放列；没有列或列已不存在（列表视图里新建、历史数据）的进第一个开放列——**兜底以项目为界**：`BoardView` 先从全局任务 store 里筛出 `project_id` 等于本项目的行，再套泳道规则，所以「回落到第一个开放列」只对本项目的任务生效，收件箱任务与别的项目的任务不会出现在这块看板上（与旁边列表视图的口径一致）；在本项目内，任务永远不会因为「没被拖过」而从看板上消失。取消完成不做写入：任务的 `column_id` 一直留着，于是它自动回到原来那一列。拖拽仍是唯一的写路径（`board:moveTask`：改 `column_id` + 按 `is_done` 联动 `completed_at` + 写排序键），拖入完成列即完成、拖出即取消完成，与列表侧的勾选完全对齐。
...
> 完整 DDL 见 `src-tauri/migrations/V2__schema.sql`：上述实体 + 索引（`tasks` 按 project/column/due_at/completed_at、`board_columns` 按 project+position、`comments`/`time_entries` 按 task_id、`time_entries` 另按 started_at、`task_tags` 按 tag_id）+ FTS5 外部内容表 `task_search(title,note)` 与 `comment_search(body)`（trigram 分词，insert/update/delete 触发器同步）。软删除行仍留在 FTS 索引，查询需按 `deleted_at IS NULL` 过滤。后续 schema 变更新增迁移、不改旧迁移（任务层级新增的 `idx_tasks_parent` / `idx_tasks_parent_completed` 见下方 V7/V9 两条）。
>
...
>
> **提醒（R-01/R-02）**：V3 迁移增加去重标记表 `task_reminders(task_id, kind, sent_at)`（主键 `(task_id, kind)`，随任务硬删级联）。`scheduler.rs` 的后台线程每 30s 扫描一次：对未完成、未软删、有 `due_at` 的任务，在提前 1 小时 / 10 分钟 / 到期时刻各触发一次提醒；标记持久化，跨扫描与重启均不重复。停机补扫时逾期任务只补发到期提醒；截止超过 24 小时的陈年逾期不再提醒。**一轮扫描 `tasks` 就覆盖整棵树**：子任务就是任务行，候选条件（未软删、未完成、`due_at` 落在扫描窗口内）对顶层任务与子任务一视同仁，`due_kinds(now, due_at)` 只算一遍。**子任务的提醒独立于父任务**（R7c §9.3）：父任务完成不再静默子任务，子任务照自己的到期时间照常提醒；父任务被软删时子任务随级联软删离开候选，而不是靠第二道查询过滤。系统通知的标题始终是「Ordo 任务提醒」，正文里的主题就是该行自己的标题，事件里的 `taskId` 也是该行自己的 id——前端点击定位因此无需按层级分支。每次触发同时做两件事：以 `reminder:triggered` 事件广播给前端（应用内 toast），并通过 `tauri-plugin-notification` 直接发送系统通知（Rust 侧直发，不经 webview，后台/托盘可达；capability 为 `notification:default`）。点击定位：插件桌面端不暴露点击回调，点击系统通知由 OS 聚焦应用窗口；前端把窗口隐藏期间触发的提醒挂起为 pending，在下一次 window `focus` 时打开任务查看器定位该任务（`features/tasks/reminders.ts`）。
>
...
> **任务层级（V7/V8/V9）**：`subtasks` 并入 `tasks`，成为单层自引用——`tasks.parent_task_id`（可空，`REFERENCES tasks(id)`），子任务不再有独立表、命令、依赖边与提醒标记。**不重建 `tasks` 的原因**：`task_search` 是按 rowid 记录的外部内容表（FTS5 external content），重建会打散索引并丢掉同步触发器，所以 V7 只做 `ALTER TABLE ADD COLUMN` + 迁移 `INSERT`。搬迁映射：`project_id` 跟父任务走（子任务不跨项目）、`column_id` 与 `repeat_rule` 留空（子任务不上看板、默认不重复）；`done` 是布尔而 `completed_at` 是时刻，**取 `updated_at` 作完成时刻的最近似值**（它必然 ≥ 真正完成的那一刻，且就是该行最后一次写入），`created_at` 兜底。依赖边（`subtask_dependencies` → `task_dependencies`）与**提醒去重标记原样搬迁**（`subtask_reminders` → `task_reminders`，值就是搬迁后的任务 id）——不原样搬的话「提前 1 小时 / 10 分钟 / 到期」会全部重发一遍。七个 `subtask:*` 命令（list / listAll / create / update / complete / delete / reorder）退役，改走任务命令；新增 `task:reorder`（在兄弟集合内移动，返回该列表的新顺序）。层级只有一层：子任务下不能再挂子任务，有子任务（**含回收站里的**）的任务不能再变成别人的子任务——被软删的子任务会随父任务的恢复一起回来（`set_children_deleted` 按状态翻转整批子行），只看存活子行会让这条路径长出三层。
>
> **孤儿清理（V8）**：V7 照搬 `subtasks` 行时没校验父任务的死活，「存活子任务挂在已软删父任务下」的行因此照搬了进来——父任务看不见，它们却出现在 `task:list` 里、还会触发提醒。`V8__no_orphan_children.sql` 把这类子任务的 `deleted_at` 补成父任务自己的删除戳（父子同刻消失）；已经有删除戳的子任务不动，它们本来就是被软删的。**导入路径执行同一条规则**（`repositories::backup::delete_orphan_children`，与插入同一个事务）：备份文档是按原样写入的，pre-V7 文档的 `subtasks` 映射取父任务的 `project_id` 却取子任务自己的 `deleted_at`，同样能造出这种孤儿。
>
> **统计的索引（V9）**：两个统计只数顶层任务（见 ST-01）后，趋势查询多了一个 `parent_task_id IS NULL` 等值谓词。等值比范围更省，规划器于是改用 `idx_tasks_parent` 整索引扫描，丢掉 `completed_at` 的范围 seek（查询计划断言当场失败）。`V9__stats_top_level_index.sql` 补一条复合索引 `idx_tasks_parent_completed(parent_task_id, completed_at)`——等值列前置、范围列后置，把「顶层任务 + 时间范围」放回同一次 seek；只按 `completed_at` 建的部分索引规划器依然不选。原 `idx_tasks_completed_at` 保留为全表口径的通用索引，`idx_tasks_parent` 保留（前缀查询已由复合索引覆盖，但它本身更窄，仍是 `list_by_parent` 一类查询的自然选择）。
